...
int ParsedRequest_parse(struct ParsedRequest *parse, const char *buf,
     ...
     if (parse->buf != NULL) {
          printf("parse object already assigned to a request\n");
          return -1;
     }

     if (buflen < MIN_REQ_LEN || buflen > MAX_REQ_LEN) {
          printf("invalid buflen %d", buflen);
          return -1;
     }
     ...
     if (index == NULL) {
          printf("invalid request line, no end of header\n");
          free(tmp_buf);
          return -1;
     }
     ...
     if (parse->method == NULL) {
          printf("invalid request line, no whitespace\n");
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
          return -1;
     ...
     if (strcmp(parse->method, "GET")) {
          printf("invalid request line, method not 'GET': %s\n",
                parse->method);
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
     ...
     if (full_addr == NULL) {
          printf("invalid request line, no full address\n");
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
          return -1;
     ...
     if (parse->version == NULL) {
          printf("invalid request line, missing version\n");
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
          return -1;
     ...
     if (strncmp(parse->version, "HTTP/", 5)) {
          printf("invalid request line, unsupported version %s\n",
                parse->version);
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
     ...
     if (parse->protocol == NULL) {
          printf("invalid request line, missing host\n");
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
          return -1;
     ...
     if (strlen(parse->host) == abs_uri_len) {
          printf("invalid request line, missing absolute path\n");
          free(tmp_buf);
          free(parse->buf);
          parse->buf = NULL;
          return -1;
     ...
     if (parse->path == NULL) { // replace empty abs_path with "/"
          ...
          printf("invalid request line, path cannot begin "
     ...
     if (parse->port != NULL) {
          ...
          if (port == 0 && errno == EINVAL) {
               printf("invalid request line, bad port: %s\n", parse->port);
               free(tmp_buf);
               free(parse->buf);
               free(parse->path);
               parse->buf = NULL;
